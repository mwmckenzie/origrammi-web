...
/**
 * Represents the various modes available in the application.
 *
 * The `appModes` object provides a mapping of keys to their respective string values
 * that define different operational states of the application.
 *
 * Keys and values include:
 * - `undefined`: Represents an undefined or uninitialized state.
 * - `authoring`: Represents the mode where users can create or edit content.
 * - `tutorial`: Represents the mode where users can access instructional or guided content.
 * - `browsing`: Represents the mode where users can explore or navigate through content.
 */
const appModes = {
    undefined: undefined,
    authoring: "authoring",
 ...
 * @property {string|undefined} mode - The mode in which the application is currently running. Default is undefined.
...
function createVisualizationSelectDropdown() {
    return $(SELECT_TAG, {
        ...
        html: Object.keys(visualizationModes).map(wrapInOptionTag).join("")
...
function createGenerateCountSelectDropdown(elemToAppendTo) {
    return $(SELECT_TAG, {
        id: "generate-count",
        html: [1, 2, 3, 4, 5, 7, 10, 15, 20, 30, 60, 100].map(wrapInOptionTag).join(""),
    }).appendTo(elemToAppendTo).change(function () {
...
function createGrammarSelectControl(elemToAppendTo) {
    return $(SELECT_TAG, {
        id: "grammar-select",
        html: testGrammars !== undefined ? Object.keys(testGrammars).map(function (item) {
            return OPTION_TAG_START + item + OPTION_TAG_END;
        }).join("") : ""
    }).appendTo(elemToAppendTo).change(function () {
        var grammarName = $(this).val();
        loadGrammar(testGrammars !== undefined ? testGrammars[grammarName] : null);
        generate();
    });
}
...
function setMode(mode) {
    ...
    console.log("Setting Application mode " + currentMode);

    app.editMode = editModes.json;
